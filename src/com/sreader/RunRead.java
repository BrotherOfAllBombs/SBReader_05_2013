...
public class RunRead extends Activity{
    ...
    private static final int CACHE_RESERVE = 10;
    private static final float BRIGHT_MAX = 100.0f;
    private static final float BRIGHT_MIN = 0.2f;
  ...
  ///////////////////////////////////////////////////////������������� ����������//////////////////////////////////////////////////////////////
  	private void initialVariable(){
  	  ...
  	  boolean auto = Utils.getAutoBrightness(this, false);
  	  float bright = Utils.getBrightnessValue(this, BRIGHT_MIN);
  	  
  	  Utils.setBrightness(this, auto, bright);
      ...
      listView1.setOnScrollListener(new android.widget.AbsListView.OnScrollListener() {
  		...
  		public void onScroll(AbsListView view, int firstVisibleItem,
  			...
  			text_readpersent_alltext.setText("��������� "+set_persent+" %");
  	...
  	mGestureDetector = new GestureDetector(new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean  onScroll (MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
			...
			Preferences.setSpeedText(time);
     		times.setText("�������� "+String.valueOf(60*1000/time)+" ����/���");
        	return true;
        }
    });
    ...
    //	text_torun.setTextColor(Color.parseColor(color));
		...
		text_readpersent.setText("��������� "+progres_pb_hor+" %");
   		id_word++;	
    }
	...
	AlertDialog.Builder dialog = null;
		...
		break;
			...
			case ID_DIALOG_BRIGHTNESS:
				LayoutInflater inflaterBright = (LayoutInflater)this.getSystemService(LAYOUT_INFLATER_SERVICE);
				View layoutBright = inflaterBright.inflate(R.layout.dialog_setbrightness, (ViewGroup)findViewById(R.id.dialogbrightnessroot));
				final CheckBox cbAuto = (CheckBox) layoutBright.findViewById(R.id.checkboxautobrightness);
				final SeekBar sbBright = (SeekBar) layoutBright.findViewById(R.id.seeksetbrightness);
				boolean auto = (Settings.System.getInt(cResolver, 
				...
				sbBright.setProgress((int)((value - BRIGHT_MIN)*BRIGHT_MAX));
				
				AlertDialog.Builder dialogBright = new AlertDialog.Builder(this);
				dialogBright.setTitle(R.string.dialog_setfont_title)
				.setView(layoutBright)
				.setPositiveButton("���������", new OnClickListener(){

				public void onClick(DialogInterface dialog, int which) {
					dialog.cancel();
					Utils.setBrightness(RunRead.this, cbAuto.isChecked(), BRIGHT_MIN + sbBright.getProgress()/BRIGHT_MAX);
					Utils.saveBrightness(RunRead.this, cbAuto.isChecked(), BRIGHT_MIN + sbBright.getProgress()/BRIGHT_MAX);
				}
...
private void setColorSheme(){
	 ...
	 private void getContentBook(){
		 ...
		 public class LoadContentBook extends AsyncTask <String, String, String>{
		    	...
		    	protected void onPreExecute() {
		    		...
		    		dialogSearch.setTitle("��������� ������");
		    		dialogSearch.setMessage("���������, ������...");
		 ...
		 private void printAllText(){
				...
				task.execute(new String[] {""});
				addTextBeforeCurrentToArr taskBefore = new addTextBeforeCurrentToArr(this);
				taskBefore.execute(new String[] {""});
		    }
	...
	public class addTextToArr extends AsyncTask<String, String, String> {
		...
		private void addWord(final String word)
		{
			runOnUiThread(new Runnable(){
				...
				{
					words.add(word);
					((MyCustomAdapter)listView1.getAdapter()).notifyDataSetChanged();
				}
			});
		}
		
		private void addChapter(final SetChapter chapter)
		{
			runOnUiThread(new Runnable(){
				public void run()
				{
					setChapters.add(chapter);
				}
			});
		}
		
		private void addWordAndChapter(final String word, final SetChapter chapter)
		{
			runOnUiThread(new Runnable(){
				public void run()
				{
		...
		private void recalculateAllText()
		{
			int pos = getPositionToScroll();
			initialseekbarAllText(pos);
		}
		...
		protected String doInBackground(String... urls) {
			...
			int set_id_word = 0;
			int set_col_word = 0;
			for (int i = 0; i < id_chapter; i++)
			{
				set_col_word += book.get(i).getWords().size();
			}
			try {
				for (Chapter curentchapter : book) {
					Log.d("addText", "Set_id_chapter is " + set_id_chapter + ", id_chapter is " + id_chapter);
					if (set_id_chapter >= id_chapter)
					{
						/*addWord(curentchapter.getName() + "#");
						addChapter(new SetChapter(set_id_chapter, set_id_word,
								set_col_word));*/
						addWordAndChapter(curentchapter.getName() + "#", new SetChapter(set_id_chapter, set_id_word,
								set_col_word));
						...
						for (int i = 0; i < count_words; i++) {
							...
							while (!checkAdd) {
								if (((set_id_chapter == id_chapter) && (i == id_word + CACHE_RESERVE)) ||
									((set_id_chapter == id_chapter +1) && (!published)))
								{
								...
								if (i + 1 < count_words) {
									...
									if (!(Utils.getTextWidth(textview, strToAdd
											+ curentchapter.getWord(i + 1),
											Utils.getWidthDisplay(context)
													- textview.getPaddingLeft()
													- textview.getPaddingLeft()))) {
										i++;
										set_col_word++;
										strToAdd = strToAdd + " "
												+ curentchapter.getWord(i);
									} else {
										checkAdd = true;
										//strToAdd += " " + set_id_chapter + " " + set_id_word + " " + set_col_word;
										/*addWord(strToAdd);
										addChapter(new SetChapter(
												set_id_chapter, set_id_word,
												set_col_word));*/
										addWordAndChapter(strToAdd, new SetChapter(set_id_chapter, set_id_word,
												set_col_word));
										strToAdd = "";
									}
								} else {
									...
									//strToAdd += " " + set_id_chapter + " " + set_id_word + " " + set_col_word;
									/*addWord(strToAdd);
									addChapter(new SetChapter(set_id_chapter,
											set_id_word, set_col_word));*/
									addWordAndChapter(strToAdd, new SetChapter(set_id_chapter, set_id_word,
											set_col_word));
									strToAdd = "";
								}
	...
	public class addTextBeforeCurrentToArr extends AsyncTask<String, String, String> {
		Context context;
		//start pos is 1 to save the title on the start of the file
		final int POS_WORD = 1;
		final int POS_CHAPTER = 1;
		
		ArrayList<SetChapter> localChapters = new ArrayList<SetChapter>();
		ArrayList<String> localWords = new ArrayList<String>();

		public addTextBeforeCurrentToArr(Context c) {
			this.context = c;
		}
		...
		private void addWord(final String word)
		{
			runOnUiThread(new Runnable(){
				public void run()
				{
					words.add(POS_WORD, word);
					//countWord++;
					//int pos = listView1.getFirstVisiblePosition() + 1;
					((MyCustomAdapter)listView1.getAdapter()).notifyDataSetChanged();
					//listView1.setSelectionFromTop(pos, 0);
					int pos_scroll=getPositionToScroll();
				    listView1.setSelection(pos_scroll);
				    initialseekbarAllText(pos_scroll);
				}
		...
		private void addChapter(final SetChapter chapter)
		{
			runOnUiThread(new Runnable(){
				public void run()
				{
					setChapters.add(POS_CHAPTER, chapter);
					//countChapter++;
				}
			});
		}
		...
		private void addWordAndChapter(final String word, final SetChapter chapter)
		{
			if (CURRENT_VISIBLE == Constants.VISIBLE_ALLTEXT)
			{
				localWords.add(0, word);
				localChapters.add(0, chapter);
			}
			else
			{
				runOnUiThread(new Runnable(){
					public void run()
					{
						...
						{
							words.addAll(POS_WORD, localWords);
							setChapters.addAll(POS_CHAPTER, localChapters);
							localWords.clear();
							localChapters.clear();
						}
						setChapters.add(POS_CHAPTER, chapter);
		...
		protected String doInBackground(String... urls) {
			...
			int set_id_chapter = id_chapter - 1;
			int set_id_word = 0;
			int set_col_word = 0;
			for (int i = 0; i < id_chapter; i++)
			{
			...
			try {
				for (set_id_chapter = id_chapter - 1; set_id_chapter >= 0; set_id_chapter--) {
					Chapter curentchapter = book.getChapter(set_id_chapter);
						int count_words = curentchapter.getWords().size();
							...
							while (!checkAdd) {
								if (i - 1 >= 0) {
									...
									if (!(Utils.getTextWidth(textview, strToAdd
										...
										set_col_word--;
										set_id_word--;
										strToAdd = curentchapter.getWord(i) + " " + strToAdd;
									} else {
										checkAdd = true;
										//strToAdd += " " + set_id_chapter + " " + set_id_word + " " + set_col_word;
										/*addWord(strToAdd);
										addChapter(new SetChapter(
												set_id_chapter, set_id_word,
												set_col_word));*/
										addWordAndChapter(strToAdd, new SetChapter(
												set_id_chapter, set_id_word,
												set_col_word));
										strToAdd = "";
									}
						...
						addChapter(new SetChapter(set_id_chapter, set_id_word,
								set_col_word));*/
						addWordAndChapter(curentchapter.getName() + "#", new SetChapter(
								set_id_chapter, set_id_word,
								set_col_word));
					//set_id_chapter--;
				}
		...
		protected void onPostExecute(String result) {
			...
			Log.d("addTextBefore", "Task completed");
			if (localWords.size() > 0)
			{
				Log.d("addTextBefore", "Cache was added on task end");
				words.addAll(POS_WORD, localWords);
				setChapters.addAll(POS_CHAPTER, localChapters);
				localWords.clear();
				localChapters.clear();
...
private int getPositionToScroll(){
	for(int i=0; i<setChapters.size(); i++){
		if (setChapters.get(i).getId_chapter()==id_chapter)
			if ((setChapters.get(i).getId_word()<=id_word) && ((setChapters.size() == i + 1) || (setChapters.get(i+1).getId_word()>=id_word)))
				return i;
	}
	return 0;
}
...
private void initialseekbarAllText(int pos_scroll){
	...
	text_readpersent_alltext.setText("��������� "+Math.round(pos_scroll/CoefPersentAllText)+" %");
